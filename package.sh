...
APP_ENTITLEMENTS="Launchster.entitlements"
APP=./Launchster.xcarchive/Products/Applications/Launchster.app
...
if [ "${ACTION}" == "package" ]; then
    ...
    codesign -s "${DEV_ID_APP}" -f --timestamp -o runtime \
        --entitlements "${APP_ENTITLEMENTS}" tmp_app/Launchster.app

    # Build component package
    pkgbuild --root tmp_app --identifier com.larrymtaylor.Launchster \
    ...
    productbuild --sign "${DEV_ID_INST}" --distribution dist.plist \
        --product requirements.plist Launchster.pkg
    ...
    xcrun notarytool submit Launchster.pkg --keychain-profile \
        ${DEV_KEY_PROF} --wait

    # Staple notary ticket to product
    xcrun stapler staple Launchster.pkg
